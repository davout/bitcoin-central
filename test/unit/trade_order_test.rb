...
class TradeOrderTest < ActiveSupport::TestCase
  ...
  test "asks should get matched in ppc descending order and get traded at ask price when a bid is executed" do
    ...
    ask_at_14 = TradeOrder.create! do |t|
      t.user = users(:trader2)
      t.amount = 75.0
      t.category = "buy"
      t.currency = "LRUSD"
    ...
    ask_at_12 = TradeOrder.create! do |t|
      t.user = users(:trader2)
      t.amount = 75.0
      t.category = "buy"
      t.currency = "LRUSD"
    ...
    ask_at_10 = TradeOrder.create! do |t|
      t.user = users(:trader2)
      t.amount = 75.0
      t.category = "buy"
      t.currency = "LRUSD"
  ...
  test "should correctly handle trade activation when insufficient balance with execution triggered from other order" do
    ...
    assert users(:trader1).balance(:lrusd).zero?
    assert_equal 0.99602.to_d, users(:trader1).balance(:btc)
    assert_equal 99.00398.to_d, users(:trader2).balance(:btc)
    assert_equal 25.0.to_d, users(:trader2).balance(:lrusd)
